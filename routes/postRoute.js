...
router.get('/stream/:audioID', async (req, res) => {
    try {
        ...
        const audioResult = await pool.query('SELECT uri FROM audio WHERE audioID = $1', [audioID]);
...
async function generateWaveform(audioPath) {
    return new Promise((resolve, reject) => {
        ffmpeg(audioPath)
            .toFormat('wav')
            .on('error', reject)
            .pipe(new WaveformData.WaveformDataGenerator({
                audio: {
                    channels: 1,
                    sampleRate: 44100
                },
...
router.get('/waveform/:audioID', async (req, res) => {
    try {
        const { audioID } = req.params;
        const result = await pool.query('SELECT waveform FROM audio WHERE audioID = $1', [audioID]);
...
router.post('/repost/:contentID', JWTverification, async (req, res) => {
    try {
        ...
        const originalPost = await pool.query('SELECT * FROM content WHERE contentID = $1', [contentID]);
...
router.post('/audio/process/volume/:audioID', JWTverification, async (req, res) => {
    try {
        const { audioID } = req.params;
        ...
        await pool.query('UPDATE audio SET uri = $1 WHERE audioID = $2', [outputPath, audioID]);
...
router.post('/audio/process/fade/:audioID', JWTverification, async (req, res) => {
    try {
        const { audioID } = req.params;
        ...
        await new Promise((resolve, reject) => {
            ...
            if (fadeOut) {
                command = command.audioFilters(`afade=t=out:st=-${fadeOut}:d=${fadeOut}`);
            }
...
router.post('/audio/process/equalizer/:audioID', JWTverification, async (req, res) => {
    try {
        const { audioID } = req.params;
...
router.post('/audio/process/speed/:audioID', JWTverification, async (req, res) => {
    try {
        const { audioID } = req.params;
